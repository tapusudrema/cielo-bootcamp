...
public class SqsManualContainerInstantiation {
    ...
    public ApplicationRunner sendMessageToQueueManualContainerInstantiation(SqsTemplate sqsTemplate) {
        ...
        return args -> sqsTemplate.send(to -> to.queue(FILA_SQS_AWS)
                .payload(new TipoNodo("hola", "John"))
        );
    }
*/
    @Bean
    ...
    /*@Bean
    SqsMessageListenerContainer<TipoNodo> sqsMessageListenerContainer(SqsAsyncClient sqsAsyncClient) {
        return new SqsMessageListenerContainer.Builder<TipoNodo>()
                .sqsAsyncClient(sqsAsyncClient)
                .queueNames(FILA_SQS_AWS)
